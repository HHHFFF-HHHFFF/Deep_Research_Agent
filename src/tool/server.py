...
class TCPServer(BaseModel):
    ...
    async def initialize(self, tool_names: list[str] | None = None):
        ...
        logger.info(
            f"| 📁 TCP Server base directory: {self.base_dir} with save path: {self.save_path} and contract path: {self.contract_path}"
        )
    ...
    async def register(
        ...
        tool_config = await self.tool_context_manager.register(
            tool, tool_config_dict=config, override=override, version=version, code=code
        )
    ...
    async def restore(
        ...
        """实现 `restore` 的业务逻辑。"""
        tool_config = await self.tool_context_manager.restore(
            tool_name, version, auto_initialize
        )
        if tool_config:
            self._registered_configs[tool_config.name] = tool_config
        return tool_config
    ...
    async def get_trainable_variables(
        ...
        return await self.tool_context_manager.get_trainable_variables(
            tool_name=tool_name
        )
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        """执行组件调用并返回结果。"""
        return await self.tool_context_manager(
            name, input, timeout=timeout, ctx=ctx, **kwargs
        )
