...
class AgentContextManager(BaseModel):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._faiss_service: FaissService | None = None
        self._variables_lock = asyncio.Lock()  # 更新相关状态。

    async def initialize(self, agent_names: list[str] | None = None) -> None:
        """初始化组件及其依赖资源。"""
            ...
            if agent_name in agent_configs:
                ...
                if (
                    ...
                    if (
                        ...
                        == 0
                        and agent_name in self._agent_history_versions
                    ):
    ...
    async def _load_from_registry(self) -> dict[str, AgentConfig]:
        ...
        async def register_agent_class(agent_cls: type[Agent]):
            ...
            try:
                ...
                agent_require_grad = (
                    agent_config_dict.get("require_grad", False)
                    if agent_config_dict and "require_grad" in agent_config_dict
                    else False
                )
                ...
                agent_function_calling = dynamic_manager.build_function_calling(
                    agent_name, agent_description, agent_parameters
                )
                agent_text = dynamic_manager.build_text_representation(
                    agent_name, agent_description, agent_parameters
                )
                agent_args_schema = dynamic_manager.build_args_schema(
                ...
                logger.info(
                    f"| 📝 Registered agent: {agent_name} ({agent_cls.__name__})"
                )

            except Exception as e:
                logger.error(
                    f"| ❌ Failed to register agent class {agent_cls.__name__}: {e}"
                )
                raise
    ...
    async def _load_from_code(self):
        ...
        ) -> tuple[str, dict[str, AgentConfig], AgentConfig | None] | None:
            """注册与 `register_agent_class` 对应的数据或状态。"""
            try:
        ...
        # 加载所需数据。
        tasks = [
            register_agent_class(agent_name, agent_data)
    ...
    async def build(self, agent_config: AgentConfig) -> AgentConfig:
        ...
        try:
            ...
            if agent_config.cls is None:
                raise ValueError(
                    f"Cannot create agent {agent_config.name}: no class provided. Class should be loaded during initialization."
                )

            # 说明相关实现细节。
            agent_instance = (
                agent_config.cls(**agent_config.config)
                if agent_config.config
                else agent_config.cls()
            )
    ...
    async def register(
        ...
        try:
            ...
            agent_require_grad = (
                agent_config_dict.get("require_grad", agent_instance.require_grad)
                if agent_config_dict and "require_grad" in agent_config_dict
                else agent_instance.require_grad
            )
            ...
            if agent_name not in self._agent_history_versions:
                self._agent_history_versions[agent_name] = {}
            self._agent_history_versions[agent_name][agent_config.version] = (
                agent_config
            )
            ...
            logger.info(
                f"| 📝 Registered agent config: {agent_name}: {agent_config.version}"
            )
            return agent_config
    ...
    async def list(self) -> list[str]:
        """实现 `list` 的业务逻辑。"""
        return [name for name in self._agent_configs]
    ...
    async def update(
        ...
        try:
            ...
            if agent_name not in self._agent_history_versions:
                self._agent_history_versions[agent_name] = {}
            self._agent_history_versions[agent_name][updated_config.version] = (
                updated_config
            )
    ...
    async def copy(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"| ❌ Failed to create agent instance for {original_config.cls.__name__}: {e}"
                )
                raise ValueError(
                    f"Failed to instantiate agent {original_config.cls.__name__} with provided config: {e}"
                )
            ...
            if not agent_code:
                logger.warning(
                    f"| ⚠️ Agent {new_name} is dynamic but source code cannot be extracted"
                )

            # 创建所需对象。
            agent_parameters = dynamic_manager.get_parameters(original_config.cls)
            agent_function_calling = dynamic_manager.build_function_calling(
                new_name, agent_description, agent_parameters
            )
            agent_text = dynamic_manager.build_text_representation(
                new_name, agent_description, agent_parameters
            )
            agent_args_schema = dynamic_manager.build_args_schema(
                new_name, agent_parameters
            )
        ...
        async with file_lock(file_path):
            ...
            save_data = {
                "metadata": {
                    ...
                    "num_versions": sum(
                        len(versions)
                        for versions in self._agent_history_versions.values()
                    ),
                },
                "agents": {},
            }
                    ...
                    try:
                        ...
                        for version_str, config_dict in versions_data.items():
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"| ⚠️ Failed to load agent config for {agent_name}@{version_str}: {e}"
                                )
                                continue
    ...
    async def save_contract(self, agent_names: builtins.list[str] | None = None):
        """保存与 `save_contract` 对应的数据或状态。"""
        contract = []
        names = (
            agent_names if agent_names is not None else list(self._agent_configs.keys())
        )
        for index, agent_name in enumerate(names):
            agent_info = await self.get_info(agent_name)
            if agent_info is None:
                logger.warning(
                    f"| ⚠️  Skipping agent '{agent_name}' in contract (not found or failed to create)"
                )
                continue
        ...
        logger.info(
            f"| 📝 Saved {len(contract)} agents contract to {self.contract_path}"
        )
    ...
    async def get_variables(
        ...
        for name, agent_config in agent_configs.items():
            ...
            if agent_config.cls is not None:
                agent_code = (
                    dynamic_manager.get_full_module_source(agent_config.cls) or ""
                )
    ...
    async def set_variables(
        ...
        async with self._variables_lock:
            ...
            if "variables" not in variable_updates:
                raise ValueError(
                    f"variable_updates must contain 'variables' field with agent code, got: {list(variable_updates.keys())}"
                )

            new_code = variable_updates["variables"]
            if not isinstance(new_code, str):
                raise TypeError(f"Agent code must be a string, got {type(new_code)}")
            ...
            try:
                agent_cls = dynamic_manager.load_class(
                    new_code, class_name=class_name, base_class=Agent, context="agent"
                )
            except Exception as e:
