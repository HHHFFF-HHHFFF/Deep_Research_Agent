...
class ModelConfig(BaseModel):
    ...
    api_key: str | None = Field(
        default=None, description="模型密钥。", repr=False, exclude=True
    )
    ...
    plugins: list[dict[str, Any]] | None = Field(
        default=None, description="提供方扩展插件。"
    )
    max_completion_tokens: int | None = Field(
        default=None, description="最大输出词元数。"
    )
    max_output_tokens: int | None = Field(
        default=None, description="响应接口最大输出词元数。"
    )
    timeout_seconds: float = Field(default=60.0, gt=0, description="请求超时秒数。")
    max_retries: int = Field(default=2, ge=0, description="客户端重试次数。")
    max_tokens_parameter: str = Field(
        default="max_tokens", description="兼容接口使用的词元参数名。"
    )
...
class LLMExtra(BaseModel):
    ...
    data: dict[str, Any] | None = Field(
        default=None, description="调用用量、原始响应等附加数据。"
    )
    parsed_model: BaseModel | None = Field(
        default=None, description="校验后的结构化响应。"
    )
