...
class ResponseOpenAI(BaseModel):
    ...
    temperature: float | None = None  # 处理模型调用。
    ...
    def _get_usage(self, response: Any) -> dict[str, Any] | None:
        """实现 `_get_usage` 的业务逻辑。"""
        usage = None
        try:
    ...
    def _get_reasoning(self, response: Any) -> str | None:
                    ...
                    for item in output:
                        ...
                        if item_type == "reasoning":
                            ...
                            if reasoning is None:
                                reasoning = None
                                break
                elif isinstance(output, dict) and output.get("type") == "reasoning":
                    reasoning = (
                        output.get("content")
                        or output.get("text")
        ...
        **kwargs: Any,
        ...
        if response_format:
            if isinstance(response_format, type) and issubclass(
                ...
                # 说明相关实现细节。
                # 转换并规范化数据。
                optimized = OpenAIChatSerializer.serialize_response_format(
                    response_format
                )
                # 转换并规范化数据。
                ...
                if "text" in response_format:
                    ...
                    "type" in response_format
                    and "name" in response_format
    ...
    async def _format_response(
        ...
        try:
            ...
            # 组装并返回结果。
                ...
                return LLMResponse(success=True, message=output_text, extra=extra)
