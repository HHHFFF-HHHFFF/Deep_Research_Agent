...
class ModelManager:
    ...
    async def _initialize_openai_models(self):
        """实现 `_initialize_openai_models` 的业务逻辑。"""
        # 处理模型调用。
        chat_models = [
            {
        ...
        for model in response_models:
            config = ModelConfig(
                ...
                supports_streaming=False,  # 组装并返回结果。
                supports_functions=False,  # 组装并返回结果。
                supports_vision=True,
                output_version=None,
                fallback_model=model.get("fallback_model"),
            )
    ...
    async def _initialize_openrouter_models(self):
        """实现 `_initialize_openrouter_models` 的业务逻辑。"""
        chat_models = [
            # 处理模型调用。
            {
    ...
    async def _initialize_anthropic_models(self):
        """实现 `_initialize_anthropic_models` 的业务逻辑。"""
        chat_models = [
            # 说明相关实现细节。
            # {
            # 处理模型调用。
            ...
            # },
            {
                ...
                "model_id": "claude-3-7-sonnet-20250219",  # 处理模型调用。
                "model_type": "chat/completions",
                "temperature": self.default_temperature,
                "max_completion_tokens": self.max_tokens,
            ...
            },
            {
                ...
                "model_id": "claude-sonnet-4-20250514",  # 处理模型调用。
                "model_type": "chat/completions",
                "temperature": self.default_temperature,
                "max_completion_tokens": self.max_tokens,
                "fallback_model": "anthropic/claude-sonnet-4.5",
            ...
            # },
            {
                ...
                "model_id": "claude-sonnet-4-5-20250929",  # 处理模型调用。
                "model_type": "chat/completions",
                "temperature": self.default_temperature,
                "max_completion_tokens": self.max_tokens,
                "fallback_model": "anthropic/claude-sonnet-4.5",
    ...
    async def _initialize_google_models(self):
        """实现 `_initialize_google_models` 的业务逻辑。"""
        chat_models = [
            {
