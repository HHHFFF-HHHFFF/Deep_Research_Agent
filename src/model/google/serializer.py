...
class GoogleChatSerializer:
    ...
    def _serialize_content_part_image(part: ContentPartImage) -> dict[str, Any]:
        ...
        if image_url.startswith("data:"):
            ...
            # 说明相关实现细节。
            raise ValueError(
                f"Google Gemini API only supports base64-encoded images or local files. Got: {image_url}"
            )
            ...
            for part in content:
                if part.type == "text":
                    serialized_parts.append(
                        GoogleChatSerializer._serialize_content_part_text(part)
                    )
                elif part.type == "image_url":
                    serialized_parts.append(
                        GoogleChatSerializer._serialize_content_part_image(part)
                    )
        ...
        if isinstance(response_format, dict):
            ...
            if "response_schema" in response_format:
                return {
                    ...
                    "response_mime_type": response_format.get(
