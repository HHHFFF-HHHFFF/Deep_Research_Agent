...
class ChatGoogle(BaseModel):
    ...
    def get_client(self, system_instruction: str | None = None):
    ...
    async def _build_params(
        ...
        **kwargs: Any,
        ...
        # 转换并规范化数据。
        system_instruction, gemini_contents = GoogleChatSerializer.serialize_messages(
            messages
        )
        ...
        if response_format:
            try:
                response_format_config = GoogleChatSerializer.serialize_response_format(
                    response_format
                )
    ...
    async def _call_model(
        ...
        **kwargs: Any,
        ...
        if stream:
            ...
            # 说明相关实现细节。
            logger.warning(
                "Streaming is not yet fully implemented for Google Gemini API"
            )
    ...
    async def _async_generate_content(self, client, **kwargs):
        ...
        return await loop.run_in_executor(
            None, lambda: client.generate_content(**kwargs)
        )
