...
async def firecrawl_fetch_url(url: str, timeout: int = DEFAULT_FETCH_TIMEOUT):
    ...
    try:
        ...
        # 说明相关实现细节。
        response = await asyncio.wait_for(app.scrape(url), timeout=timeout)
...
async def fetch_crawl4ai_url(url: str, timeout: int = DEFAULT_FETCH_TIMEOUT):
    ...
    try:
        async with AsyncWebCrawler() as crawler:
            # 说明相关实现细节。
            response = await asyncio.wait_for(crawler.arun(url=url), timeout=timeout)
...
async def fetch_url(
    url: str, timeout: int = DEFAULT_FETCH_TIMEOUT
