...
class FaissSearchRequest(BaseModel):
    ...
    filter: dict[str, Any] | Callable | None = Field(
        None, description="Filter by metadata"
    )
    fetch_k: int = Field(
        20, ge=1, le=10000, description="Number of documents to fetch before filtering"
    )
...
class FaissAddRequest(BaseModel):
    ...
    ids: list[str] | None = Field(None, description="Custom IDs for each text")
...
class FaissConfig(BaseModel):
    ...
    distance_strategy: str = Field(
        "cosine", description="Distance strategy (euclidean, cosine, max_inner_product)"
    )
